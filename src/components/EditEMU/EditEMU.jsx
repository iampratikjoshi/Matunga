...
const Edit = () => {
    ...
    const handleDeleteDivisionEMU = async (wheelId) => {
        ...
        if (confirmDelete) {
            try {
                ...
                const response = await api.delete(`/emudivision/deletedata/${wheelId}`, {
                    method: "DELETE",
                });

                if (response.status === 200 || response.status === 204) {
                    alert("Item deleted successfully!");
                    navigate("/parentemuedit/editemu");
    ...
    const handleDeleteFinalEMU = async (wheelId) => {
        ...
        if (confirmDelete) {
            try {
                // Make an API call to delete the item
                const response = await api.delete(`/emufinalinspection/deletedata/${wheelId}`, {
                    method: "DELETE",
                });
    ...
    const handleDeletePressOffEMU = async (wheelId) => {
        ...
        if (confirmDelete) {
            try {
                // Make an API call to delete the item
                const response = await api.delete(`/emupressoff/deletedata/${wheelId}`, {
                    method: "DELETE",
                });
    ...
    const handleDeletePreInspectionEMU = async (wheelId) => {
        ...
        if (confirmDelete) {
            try {
                // Make an API call to delete the item
                const response = await api.delete(`/emuscheduledpreinspection/deletedata/${wheelId}`, {
                    method: "DELETE",
                });
                                        ...
                                        <tr key={item.WheelId}>
                                            ...
                                            <td>
                                                <FontAwesomeIcon
                                                    ...
                                                    onClick={() =>
                                                        navigate(
                                                            "/parentemuedit/UpdateEMUSchedulePreInspectionForm/details",
                                                            {
                                                                state: {
                                                ...
                                                <FontAwesomeIcon
                                                    icon={faTrash}
                                                    onClick={() => handleDeletePreInspectionEMU(item.WheelId || item.wheelid)}
                                                    className="icon-hover"
                                        ...
                                        <tr key={item.WheelId || item.wheelid}>
                                            ...
                                            <td>
                                                <FontAwesomeIcon
                                                    ...
                                                    onClick={() =>
                                                        navigate(
                                                            "/parentemuedit/UpdateEMUFinalInspection/axle_details",
                                                            {
                                                                state: {
                                                                    WheelNo: item.WheelNo || item.ShopSrNumber,
                                                ...
                                                <FontAwesomeIcon
                                                    icon={faTrash}
                                                    onClick={() => handleDeleteFinalEMU(item.WheelId || item.wheelid)}
                                                    className="icon-hover"
                                                    style={{ color: 'red' }} // Optional: style for the delete icon
                                                />
                                    ...
                                    {results.pressOn.map((item) => (
                                        <tr key={item.WheelId || item.wheelid}>
                                            ...
                                            <td>
                                                ...
                                                <FontAwesomeIcon
                                                    icon={faTrash}
                                                    onClick={() => handleDeletePressOnEMU(item.WheelId || item.wheelid)}
                                                    className="icon-hover"
                                                    style={{ color: 'red' }} // Optional: style for the delete icon
                                                />
                                    ...
                                    {results.pressOff.map((item) => (
                                        <tr key={item.WheelId || item.wheelid}>
                                            ...
                                            <td>
                                                ...
                                                <FontAwesomeIcon
                                                    icon={faTrash}
                                                    onClick={() => handleDeletePressOffEMU(item.WheelId || item.wheelid)}
                                                    className="icon-hover"
                                                    style={{ color: 'red' }} // Optional: style for the delete icon
                                                />
                                    ...
                                    {results.divisionPreInspection.map((item) => (
                                        <tr key={item.WheelId || item.wheelid}>
                                            ...
                                            <td>
                                                ...
                                                <FontAwesomeIcon
                                                    icon={faTrash}
                                                    onClick={() => handleDeleteDivisionEMU(item.WheelId || item.wheelid)}
                                                    className="icon-hover"
                                                    style={{ color: 'red' }} // Optional: style for the delete icon
                                                />

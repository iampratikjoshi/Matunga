...
const AllEntry = () => {
  ...
  //     worksheet.getCell(`C${currentRow + 16}`).font = { bold: true };
  //     worksheet.getCell(`E${currentRow + 16}`).font = { bold: true };
  //     //Bold Text Headers
  ...
  //     worksheet.getCell(`C${currentRow + 23}`).font = { bold: true };
  //     worksheet.getCell(`E${currentRow + 23}`).font = { bold: true };
  //     worksheet.getCell(`A${currentRow + 8}`).font = { bold: true };
  //     worksheet.getCell(`A${currentRow + 12}`).font = { bold: true };
  //     worksheet.getCell(`C${currentRow + 18}`).font = { bold: true };
  ...
  const exportToExcel = async () => {
    ...
    data.forEach((res, index) => {
      ...
      worksheet.mergeCells(currentRow + 8, 1, currentRow + 11, 2);
      // worksheet.mergeCells(currentRow + 12, 1, currentRow + 15, 2);
      worksheet.mergeCells(currentRow + 8, 4, currentRow + 9, 28);
      worksheet.mergeCells(currentRow + 10, 4, currentRow + 11, 28);
      // worksheet.mergeCells(currentRow + 12, 4, currentRow + 13, 26);
      // worksheet.mergeCells(currentRow + 14, 4, currentRow + 15, 26);
      // worksheet.mergeCells(currentRow + 16, 4, currentRow + 19, 26);
      // worksheet.mergeCells(currentRow + 16, 1, currentRow + 19, 2);
    ...
    data.forEach((row) => {
      body.push(
        [
          ...
          { content: handleNullValue(row.RodGaugeDefect), rowSpan: 2 },
        ...
        [
          ...
          { content: handleNullValue(row.DiscParticularA), colSpan: 26 }, // Data for Disc Particular A
        ],
        [
          { content: "B" }, // Subheader B
          { content: handleNullValue(row.DiscParticularB), colSpan: 26 }, // Data for Disc Particular B
        ],
          ...
          {data.map((res) => (
            <tbody name="tbody" key={`tbody-${res.id}`}>
              ...
              <td rowSpan="2">{res.CTRBStatusB}</td> */}
                ...
                <td rowSpan="2">{res.RodGaugeDefect}</td>
              ...
              <tr>
                ...
                <th colSpan="">A</th>
                <td colSpan="29">{res.DiscParticularA}</td>
              </tr>
              <tr>
                <th rowSpan="" colSpan="">
                  B
                </th>
                <td colSpan="29">{res.DiscParticularB}</td>
              </tr>
            </tbody>
          ))}
        </table>

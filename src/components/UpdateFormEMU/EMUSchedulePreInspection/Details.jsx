...
function Details({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (WheelId) {
          const response = await api.get(`/emuscheduledpreinspection/getdata/`+WheelId); // Assuming your API needs the WheelId in the endpoint
          console.log("emuscheduledpreinspection",response.data);
          ...
          setFormDataScheduleEMU((prevFormData) => ({
            ...
            RodGaugeDefect: response.data[0].RodGaugeDefect,
    ...
    fetchData();
  }, [WheelId, setFormDataScheduleEMU]);
  ...
  }, [WheelNo,WheelId, setFormDataScheduleEMU]);
  ...
  const saveandcontinue = () => {
    if (validateForm()) {
      onNextStep();
      navigate("/parentemuedit/UpdateEMUSchedulePreInspectionForm/w2_details");
    }
  };

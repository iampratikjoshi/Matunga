...
function IdentificationDetails({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (wheelid) {
          const response = await api.get(`/emupressoff/getdata/` + wheelid); // Assuming your API needs the wheelid in the endpoint
          console.log("emupressoff", response.data);
    ...
    fetchData();
  }, [wheelid, setFormDataPressOffEMU]);

...
const AllEntryFinal = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get("/emu/finalinspection/getalldata");
  ...
  const exportToExcel = async () => {
    ...
    data.forEach((formDataFinalEMU, index) => {
  ...
  const exportToPDF = () => {
    ...
    // Sample data array with multiple entries
    // const entries = [
    //   formDataFinalEMU1, // Example entry 1
    //   formDataFinalEMU2, // Example entry 2
    //   formDataFinalEMU3, // Example entry 3
    ...
    doc.autoTable({
      ...
      didDrawPage: (data) => {
        if (data.pageNumber === 1) {
          doc.setFontSize(12);
          doc.text("EMU Final Inspection Report", data.settings.margin.left, 20);
        }
    ...
    <div className="main_div">
      ...
      <div className="button_div">
        ...
        <button
          ...
          onClick={() => navigate("/parentemuedit/UpdateEMUFinalInspection/wheel_details")}
        >
          Add Entry
        </button>
        <button
